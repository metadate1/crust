...
- Cross-level transitions are high-level Rust state transitions. The host keeps all selected file
  handles but validates/materializes only the initial pair; it does not page each destination pair
  into a retail-equivalent live scene.
- `crust-renderer` implements texture decode, cache keys, projection, ordering and blend-command
  rules, and a separate WebGL2 command backend exists. The shipped live screen currently uses an
  original low-poly diagnostic renderer, not decoded retail meshes, textures, sprites, animation,
  camera draw lists or pixel-equivalent effects.
...
- The password UI applies a local deterministic progression rule, not the retail password codec.
  ...
  handshake across all transitions has not been playthrough-certified. No live level/object event
  currently creates a manual card slot, so manual saving is exercised at the Rust model/storage
  test boundary rather than through gameplay UI.
- No complete level, boss, bonus route, ending, demo, death/checkpoint sequence, long soak, mobile
  audio session, or multiple physical gamepad matrix was completed in browser verification.
